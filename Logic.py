...
class Logics:
    ...
    def sort_n_merge_by_chr(self, init_merge, init_be, ignor_list):
        ...
        aqia_a_c_dict = self.filter_out_by_ACGTU_rule(aqia_chr_dict, a_or_c_idx, a_c_rule)
        ...
        util.make_excel_after_sorting(
            work_dir + "merge_cas9_abe_cbe_top_N/merge_by_ABE_top_" + str(top_n), top_n_abe_list,
            init_be)
        util.make_excel_after_sorting(
            work_dir + "merge_cas9_abe_cbe_top_N/merge_by_CBE_top_" + str(top_n), top_n_cbe_list,
            init_be)
        ...
        for chr_aqia_key, trnscrpt_aqia_list in aqia_a_c_dict.items():
            result_aqia_list = []
            result_aqia_list = logic_prep.merge_cas9_abe_cbe_to_list(chr_aqia_key, [trnscrpt_aqia_list, abe_score_dict,
                                                                                    cbe_score_dict,
                                                                                    cs9_score_dict], result_aqia_list)
            sort_aqia_by_abe_list = logic_prep.sort_by_idx_element(result_aqia_list, -2, [])
            sort_aqia_by_cbe_list = logic_prep.sort_by_idx_element(result_aqia_list, -1, [])

            """
            # extend TOP N lists to (top_n_abe_list, top_n_cbe_list)
            it needs filter out same context seq in different trnscrpt
            """
            top_n_aqia_abe_list.extend(sort_aqia_by_abe_list[:top_n])
            top_n_aqia_cbe_list.extend(sort_aqia_by_cbe_list[:top_n])

        util.make_excel_after_sorting(
            work_dir + "merge_cas9_abe_cbe_top_N/merge_by_ABE_top_" + str(top_n) + "_aqia", top_n_aqia_abe_list,
            init_be)
        util.make_excel_after_sorting(
            work_dir + "merge_cas9_abe_cbe_top_N/merge_by_CBE_top_" + str(top_n) + "_aqia", top_n_aqia_cbe_list,
            init_be)

    def sort_n_merge_by_all(self, init_merge, init_be, ignor_list):
        ref_path = init_merge[0]
        cdf_file = init_merge[1]
        a_or_c_idx = init_merge[2]
        a_c_rule = init_merge[3]
        ...
        util.make_excel_after_sorting(
            work_dir + "merge_cas9_abe_cbe_top_N/merge_by_ABE_top_" + str(top_n) + "_all", sort_by_abe_list,
            init_be)
        util.make_excel_after_sorting(
            work_dir + "merge_cas9_abe_cbe_top_N/merge_by_CBE_top_" + str(top_n) + "_all", sort_by_cbe_list,
            init_be)
        ...
        it needs filter out same context seq in different trnscrpt
        """
        sort_aqia_by_abe_list = logic_prep.sort_by_idx_element(result_aqia_list, -2, [])[:top_n]
        sort_aqia_by_cbe_list = logic_prep.sort_by_idx_element(result_aqia_list, -1, [])[:top_n]

        util.make_excel_after_sorting(
            work_dir + "merge_cas9_abe_cbe_top_N/merge_by_ABE_top_" + str(top_n) + "_aqia_all",
            sort_aqia_by_abe_list,
            init_be)
        util.make_excel_after_sorting(
            work_dir + "merge_cas9_abe_cbe_top_N/merge_by_CBE_top_" + str(top_n) + "_aqia_all",
