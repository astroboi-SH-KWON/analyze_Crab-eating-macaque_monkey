...
class Utils:
    ...
    def make_Deep_PE_input_AQIA(self, path, data_dict):
        workbook = openpyxl.Workbook()
        sheet = workbook.active
        ...
        sheet.cell(row=row, column=5, value='Target sequence')
        ...
        for chr_key, val_dict in data_dict.items():
            for trnscrpt_id, vals_arr in val_dict.items():
                ...
                for trgt_idx in range(1, len(vals_arr)):
                    row += 1
                    sheet.cell(row=row, column=1, value=str(row - 1))
                    sheet.cell(row=row, column=2, value=chr_key)
                    sheet.cell(row=row, column=3, value=gene_name)
                    sheet.cell(row=row, column=4, value=trnscrpt_id)
                    sheet.cell(row=row, column=5, value=vals_arr[trgt_idx])

        # workbook.save(filename=path + "deep_pe_input_chr_AQIA_" + str(clock()) + self.ext_xlsx)
        workbook.save(filename=path + "deep_pe_input_chr_AQIA" + self.ext_xlsx)

    """
        make_cas_off_finder_input : .txt input file for CAS_OFF_FINDER
        :param
            seq_set_pool
            init
                pam_seq
    ...
    def make_merge_excel(self, result_path, init_dict_arr, init_len):
        full_dict = init_dict_arr[0]
        abe_score_dict = init_dict_arr[1]
        cbe_score_dict = init_dict_arr[2]
        cs9_score_dict = init_dict_arr[3]
        ...
        for chr_key, trnscrpt_val in full_dict.items():
            for trnscrpt_id, vals_arr in trnscrpt_val.items():
                ...
                for trgt_idx in range(1, len(vals_arr)):
                        ...
                        sheet.cell(row=row, column=1, value=str(row - 1))
                        sheet.cell(row=row, column=2, value=chr_key)
                        sheet.cell(row=row, column=3, value=gene_nm)
                        sheet.cell(row=row, column=4, value=description)
                        sheet.cell(row=row, column=5, value=trnscrpt_id)
                        sheet.cell(row=row, column=6, value=gene_id)
                        sheet.cell(row=row, column=7, value=strand)
                        sheet.cell(row=row, column=8, value=cntxt_seq[add_seq1_len:-add_seq2_len])
                        sheet.cell(row=row, column=9, value=cntxt_seq)
                        sheet.cell(row=row, column=10, value=cntxt_seq[add_seq1_len + spacer_len:-add_seq2_len])
                        sheet.cell(row=row, column=11, value=clvg_site)
                        sheet.cell(row=row, column=12, value=cas9_score)
                        sheet.cell(row=row, column=13, value=abe_score)
                        sheet.cell(row=row, column=14, value=cbe_score)
